...
As you might have noticed in the code snippets above, all of the components are importing a Context provider called `ThemeContext` from `App.jsx` and extracting the currently selected theme with the `useContext()` hook.

I managed to implement the theming system by combining React's **conditional rendering** with **separate stylesheets** for each theme.
...
The second class name is generated by interpolating a base name (e.g. `theme-X-logo`) with the theme number (e.g. `1`) to produce something like `theme-1-logo`, which brings to my second point.
